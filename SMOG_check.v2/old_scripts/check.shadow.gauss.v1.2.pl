...
$EXEC_SHADOW=$ENV{'EXEC_SHADOW'};
$EXEC_NAME=$ENV{'EXEC_READ'};
$JAVA=$ENV{'JAVA'};
...
unless( -e $EXEC_SHADOW){
print "Cant find the shadow executable\n";
die;
}

unless( -e $JAVA){
print "Cant find JAVA\n";
die;
}
...
## run read once to make the output files.
	print "Running the first round of READ...\n";
        `$EXEC_NAME $PDB.settings -noshift -gaussian &> $PDB.output `;

        $TAIL=`tail -1 $PDB.output | awk '{print \$1}'`;
        $ABORT=`grep -i ABORT $PDB.output | wc -l| awk '{print \$1}'`;
        chomp($TAIL);
        chomp($ABORT);
##      if(param('oform') eq "screen"){
        if($TAIL ne 'SUCCESS' or $ABORT gt 0){
		print "Read failed on the first attempt...\n";
		print "dying now\n";
                print "FAILED\n";
		die;
		$FAILED++;
	}else{
		print "READ completed without fatal errors\n";
	}


## run shadow for the output
	print "Running shadow...\n";
        $SHADOWFINISHED=0;
	`$JAVA -Xmx750m -jar $EXEC_SHADOW  -g $PDB.gro -t $PDB.top -o $PDB.contacts --reindex --default -p $PDB.meta.pdb -ch $PDB.ndx  &> shadow.output.tmp`;
        $SHADOWFINISHED=`grep Finished shadow.output.tmp | wc -l`;
...
##        `mv $PDB.meta.pdb $PDB.pdb`;
	`bash contact.bash $PDB.contacts`;	
## remake the settings file for a shadow mapped potential
	...
	open(READSET,">$PDB.settings") or die "can\'t open $PDB.settings\n";
        printf READSET ("%s.meta.pdb\n", $PDB);
        printf READSET ("%s.top\n", $PDB);
        printf READSET ("%s.gro\n", $PDB);
        printf READSET ("%s.ndx\n", $PDB);
	printf READSET ("%s\n", "All-Atom");
	printf READSET ("%s\n", "UPLOAD");
        printf READSET ("%s.contacts\n", $PDB);
        printf READSET ("%s\n", $R_CD);
        printf READSET ("%s\n", $R_P_BB_SC);
        printf READSET ("%s\n", $R_N_SC_BB);
        printf READSET ("%s\n", $PRO_DIH);
...
## rerun read
	print "Running the second round of READ...\n";

        `$EXEC_NAME $PDB.settings -gaussian &> $PDB.output `;
...
##      if(param('oform') eq "screen"){
        if($TAIL ne 'SUCCESS' or $ABORT gt 0){
                print "Read failed on the second attempt...\n";
		print "dying now\n";
                print "FAILED\n";
		die;
        }else{
...
##
	`./top.clean.bash $PDB.ndx $PDB.ndx2`;
	`mv $PDB.ndx2 $PDB.ndx`;
...
###`$EXEC_PATH settings...`;
	`./top.clean.bash $PDB.top $PDB.top2`;
	`mv $PDB.top2 $PDB.top`;
			...
			# check values for contact energy
				...
				$longcontact=0;
					...
					# determine the epsilon of the contact
                                      ...
                                      # determine the epsilon of the contact
                                        $W=$A[3];
                                        # determine the r0 of the contact
                                        $r0=$A[4];
                                        # determine the sigmaG of the contact
                                        $sigmaG=$r0*sqrt(1.0/(50.0*log(2.0)));
                                        if($A[5] < $MINTHR*$sigmaG || $A[5] > $MAXTHR*$sigmaG){
                                                $GAUSSSIGMA++;
                                        }

                                        # determine the a (rep value) of the contact
                                        if($A[6] < $MINTHR*$rep_s12 || $A[6] > $MAXTHR*$rep_s12){
                                                $GAUSSEXCL++;
                                        }


					if($r0 > 0.6){
						print "long contacts! distance $r0 nm.\n";
						print "$LINE";
		                                $longcontact++;
		...
		## check stuff
			...
			if($theta_gen_N == 0 || $phi_gen_N == 0 ||  $improper_gen_N == 0){
				...
				print "FAILED: excluded volume\n";
                        ...
                        if($GAUSSEXCL > 0){
                                print "FAILED: Gaussian excluded volume\n";
                                $FAILED++;
                        }else{
                                 print "Gaussian excluded volume: PASSED\n";
                        }               
 

                        if($GAUSSSIGMA > 0){
                                print "FAILED: Gaussian sigma\n";
                                $FAILED++;
                        }else{
                                 print "Gaussian sigma: PASSED\n";
                        }
...
#                                if($PBBvalue/$LIGdvalue > $MAXTHR*$PRO_DIH/$LIGAND_DIH || $PBBvalue/$LIGdvalue < $MINTHR*$PRO_DIH/$LIGAND_DIH){
		...
		print "number of non-ligand atoms $NUMATOMS_LIGAND total E $E_TOTAL\n";
                ...
                if($FAILED > 0){
                        ...
                        `mv shadow.output.tmp  $FAILDIR/$PDB.fail$NFAIL.shadow`;
                        `mv settings.read.tmp $FAILDIR/$PDB.fail$NFAIL.settings.out`; 
