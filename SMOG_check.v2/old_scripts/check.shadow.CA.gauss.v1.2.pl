...
$FAILDIR="FAILED.shadow.CA.gauss";
##things to do
...
## run shadow for the output
	...
	$SHADOWFINISHED=0;
	`$JAVA -Xmx750m -jar $EXEC_SHADOW  -g $PDB.gro -t $PDB.top -o $PDB.contacts --reindex --default --coarse CA -p $PDB.meta.pdb -ch $PDB.ndx &> shadow.output.tmp`;
