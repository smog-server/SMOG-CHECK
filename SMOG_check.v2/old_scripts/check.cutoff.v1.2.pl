...
$NFAIL=0;
unless( -e $EXEC_NAME){
print "Cant find the executable\n";
die;
}
...
##run the check script
        ...
        `$EXEC_NAME $PDB.settings &> $PDB.output `;
				...
				$FAILNONSTACK=0;
					...
					if($MOLTYPE[$A[0]] eq "NUCLEIC" and $MOLTYPE[$A[1]] eq "NUCLEIC" and $ATOMTYPE[$A[0]] ne "BACKBONE" and  $ATOMTYPE[$A[1]] ne "BACKBONE" and $ATOMNAME[$A[0]] ne "C1\*" and $ATOMNAME[$A[1]] ne "C1\*" and abs($RESNUM[$A[0]]-$RESNUM[$A[1]]) == 1 and $CID[$A[0]] == $CID[$A[1]]){
						...
						if($stackingE == 0 ){
							...
							$FAILSTACK++;
							print "stack $stackingE  $W $A[0] $A[1] \n";
						}
					}else{
			...
			## cross-check the angles
				...
				for($i=0;$i<$theta_gen_N;$i++){
					...
					if($angle_array{$theta_gen[$i]} != 1){
						##print "$theta_gen[$i]\n";
						$FAIL_angles++;
						print "$FAIL_angles angles failed\n";
					}
				}
...
##			}
		...
		## check stuff
			...
			print "number of atoms(excluding ligands and ion) = $NUMATOMS_LIGAND\n";
			...
			if($theta_gen_N == 0 || $phi_gen_N == 0 ||  $improper_gen_N == 0){
				print "FAILED: couldnt generate something....\n";
				$FAILED++;
			}
 ...
 #                                                               print "ring passed\n";
  #                                                      }
						...
						if($MOLTYPE[$i] eq "AMINO"){
							if($ATOMTYPE[$i] eq "BACKBONE" or  $ATOMTYPE[$i+$j] eq "BACKBONE"){
								...
								if($PSCvalue !=$ED_T[$i][$j] && $PSCvalue !=0){
									$PSCfail++;
									print "sidechain atom $i $j failed\n";
			...
			print "$NUM_NONZERO rigid dihedrals are identified\n";
                        ...
                        if($double_bond >0){
                                print "FAILED: Some bonds were assigned more than once\n";
                                $FAILED++;
                        }else{
                        ...
                        if($double_angle >0){
                                print "FAILED Some angles were assigned more than once\n";
                                $FAILED++;
                        }else{
                        ...
                        if($FAIL_angles >0){
                                print "FAILED: Something funny with the angles... not consistent between script and top.\n";
                                $FAILED++;
                        }else{
                        ...
                        if($FAIL_phi >0){
                                print "FAILED: Something funny with the dihedral angles... not consistent between script and top.\n";
                                $FAILED++;
                        }else{
			...
			if($rigid_fail >0){
                                print "FAILED: Rigid dihedrals do not have the correct strengths\n";
                                $FAILED++;
			}else{
				print "strength of rigid dihedrals: PASSED\n";
                        ...
                        if($double_dihedral >0){
                                print "FAILED: Some dihedral were assigned more than once\n";
                                $FAILED++;
                        }else{
                        ...
                        if($missing_dihedral_3 >0){
                                print "FAILED: A type 3 dihedral was present without a type 1...\n";
                                $FAILED++;
                        }else{
			...
			if($stackingE ==0){
				...
				print "FAILED: stacking interactions are not all the same value\n";
				$FAILED++;
			}else{
			...
			if($NonstackingE ==0){
				...
				print "FAILED: non-stacking interactions are not all the same value\n";
				$FAILED++;
			}else{
			...
			if($NonstackingE !=0 && $stackingE !=0){
				...
				if($CR > 3.02 || $CR < 2.98){
					print "FAILED: ratio between stacking and non stacking is not 3\n";
					print "ratio is $CR\n";
					$FAILED++;
				}else{
...
#                                if($PBBvalue/$LIGdvalue > $MAXTHR*$PRO_DIH/$LIGAND_DIH || $PBBvalue/$LIGdvalue < $MINTHR*$PRO_DIH/$LIGAND_DIH){
					...
					print "Target $RR_TARGET Actual $RR $MINTHR $MAXTHR";
                                        $FAILED++;
