...
## since this script checks for default values, we don't need to prepare the bif/sif files
...
##	`cp $TEMPLATE_DIR/sbm.b temp.bifsif/tmp.b`;
	...
	`$EXEC_NAME -i $PDB.pdb -g $PDB.gro -o $PDB.top -n $PDB.ndx -s $PDB.contacts -tAA $BIFSIF/  &> $PDB.output `;
